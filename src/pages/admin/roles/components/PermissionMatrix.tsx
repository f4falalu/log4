...
export function PermissionMatrix({ roleId }: PermissionMatrixProps) {
  ...
  const togglePermission = async (permissionId: string, currentlyAssigned: boolean) => {
    const newPermissionIds = new Set(rolePermissionIds);
    ...
    try {
      ...
      toast({
        ...
        description: `Permission ${currentlyAssigned ? 'removed from' : 'added to'} role`,
      });
    } catch (error) {
      toast({
  ...
  const selectAllInCategory = async (category: string) => {
    ...
    categoryPermissions.forEach((p) => newPermissionIds.add(p.id));
    ...
    try {
      ...
      toast({
        title: 'Category permissions added',
        description: `All ${PERMISSION_CATEGORIES[category as keyof typeof PERMISSION_CATEGORIES].label} permissions added`,
      });
    } catch (error) {
      toast({
        title: 'Error updating permissions',
  ...
  const deselectAllInCategory = async (category: string) => {
    const categoryPermissions = permissionsByCategory?.[category] || [];
    const categoryPermissionIds = new Set(categoryPermissions.map((p) => p.id));
    const newPermissionIds = new Set(
      Array.from(rolePermissionIds).filter((id) => !categoryPermissionIds.has(id))
    );
    ...
    try {
      ...
      toast({
        title: 'Category permissions removed',
        description: `All ${PERMISSION_CATEGORIES[category as keyof typeof PERMISSION_CATEGORIES].label} permissions removed`,
      });
    } catch (error) {
      toast({
        title: 'Error updating permissions',
  ...
  const assignedCount = rolePermissionIds.size;
      ...
      <CardContent>
        <div className="space-y-4">
          {Object.entries(filteredCategories).map(([category, permissions]) => {
            ...
            const allSelected = categoryAssignedCount === permissions.length;
