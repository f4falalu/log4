/**
 * DriverInfoCard Component
 *
 * Right-side info card showing driver details when clicked on map
 * Reference: Rentizy property detail card pattern
 *
 * Design: Clean card layout with driver stats and credentials
 */

import { X, User, Phone, Mail, CreditCard, Briefcase, MapPin, TrendingUp } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Separator } from '@/components/ui/separator';
import type { Driver } from '@/types';

interface DriverInfoCardProps {
  driver: Driver;
  onClose: () => void;
}

/**
 * Info row component
 */
function InfoRow({
  icon: Icon,
  label,
  value,
  className,
}: {
  icon?: any;
  label: string;
  value: string;
  className?: string;
}) {
  return (
    <div className={`flex items-start gap-3 ${className || ''}`}>
      {Icon && <Icon className="h-4 w-4 mt-0.5 text-muted-foreground" />}
      <div className="flex-1 min-w-0">
        <div className="text-xs text-muted-foreground">{label}</div>
        <div className="text-sm font-medium text-foreground">{value}</div>
      </div>
    </div>
  );
}

/**
 * Section component
 */
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-foreground">{title}</h4>
      {children}
    </div>
  );
}

/**
 * Get status badge
 */
function getStatusBadge(status?: string) {
  const statusMap: Record<string, { variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
    active: { variant: 'default' },
    available: { variant: 'secondary' },
    on_route: { variant: 'default' },
    offline: { variant: 'outline' },
    inactive: { variant: 'destructive' },
  };

  const config = status ? statusMap[status] || { variant: 'outline' as const } : { variant: 'outline' as const };
  const label = status ? status.replace('_', ' ').replace(/\b\w/g, (l) => l.toUpperCase()) : 'Unknown';

  return <Badge variant={config.variant}>{label}</Badge>;
}

/**
 * Driver Info Card
 */
export function DriverInfoCard({ driver, onClose }: DriverInfoCardProps) {
  // Format location if available
  const location = driver.currentLocation || 'Location unavailable';

  return (
    <div className="fixed right-0 top-0 h-full w-[360px] bg-background border-l border-border z-[1100] flex flex-col overflow-hidden animate-slide-in-right">
      {/* Header */}
      <div className="p-4 border-b border-border flex items-center justify-between bg-muted/30">
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-foreground truncate">
            {driver.name}
          </h3>
          <div className="mt-1">{getStatusBadge(driver.status)}</div>
        </div>
        <button
          onClick={onClose}
          className="p-2 rounded hover:bg-muted transition-colors flex-shrink-0 ml-2"
          aria-label="Close panel"
        >
          <X className="w-5 h-5" />
        </button>
      </div>

      {/* Content */}
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {/* Contact Information */}
        <Section title="Contact">
          <InfoRow
            icon={Phone}
            label="Phone"
            value={driver.phone}
          />
          {driver.email && (
            <InfoRow
              icon={Mail}
              label="Email"
              value={driver.email}
            />
          )}
        </Section>

        <Separator />

        {/* License Details */}
        <Section title="License">
          <InfoRow
            icon={CreditCard}
            label="License Number"
            value={driver.licenseNumber || 'N/A'}
          />
          <InfoRow
            label="Type"
            value={driver.licenseType === 'commercial' ? 'Commercial' : 'Standard'}
          />
          {driver.licenseExpiry && (
            <InfoRow
              label="Expiry Date"
              value={new Date(driver.licenseExpiry).toLocaleDateString()}
            />
          )}
          {driver.licenseVerified && (
            <div className="flex items-center gap-2 text-sm text-green-500">
              <div className="h-2 w-2 rounded-full bg-green-500" />
              <span>Verified</span>
            </div>
          )}
        </Section>

        <Separator />

        {/* Employment Details */}
        <Section title="Employment">
          {driver.employer && (
            <InfoRow
              icon={Briefcase}
              label="Employer"
              value={driver.employer}
            />
          )}
          {driver.position && (
            <InfoRow
              label="Position"
              value={driver.position}
            />
          )}
          {driver.groupName && (
            <InfoRow
              label="Group"
              value={driver.groupName}
            />
          )}
        </Section>

        <Separator />

        {/* Current Status */}
        <Section title="Current Status">
          <InfoRow
            icon={MapPin}
            label="Location"
            value={location}
          />
          {driver.assignedVehicleId && (
            <InfoRow
              label="Assigned Vehicle"
              value={driver.assignedVehicleId}
            />
          )}
        </Section>

        <Separator />

        {/* Performance Stats (Placeholder) */}
        <Section title="Performance">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 rounded-lg bg-muted/30 border border-border">
              <div className="text-xs text-muted-foreground">Deliveries</div>
              <div className="text-2xl font-bold text-foreground mt-1">-</div>
            </div>
            <div className="p-3 rounded-lg bg-muted/30 border border-border">
              <div className="text-xs text-muted-foreground">Rating</div>
              <div className="text-2xl font-bold text-foreground mt-1 flex items-center gap-1">
                - <TrendingUp className="h-4 w-4 text-green-500" />
              </div>
            </div>
          </div>
        </Section>
      </div>

      {/* Actions */}
      <div className="p-4 border-t border-border flex gap-2 bg-muted/30">
        <Button variant="outline" className="flex-1" size="sm">
          View History
        </Button>
        <Button variant="default" className="flex-1" size="sm">
          Assign Route
        </Button>
      </div>
    </div>
  );
}
