 ...
 * Intelligent facility-to-slot allocation with weight-based prioritization
 */

import type { Vehicle, VehicleSlot, SlotAssignment } from './slotMapper';
import { generateVehicleSlotMap, validateSlotAssignment } from './slotMapper';
...
export function autoAssignFacilitiesToSlots(
  ...
  // Check if all facilities can fit
  if (facilities.length > availableSlots.length && !allowOverflow) {
    result.errors.push(
      `Cannot fit ${facilities.length} facilities into ${availableSlots.length} slots`
    );
    result.unassigned = [...facilities];
    return result;
  }
  ...
  // Group slots by tier (for tier-based assignment)
  ...
  for (const facility of sortedFacilities) {
    ...
    if (!assigned) {
      ...
      if (!allowOverflow) {
        result.errors.push(`Could not assign facility ${facility.id} - no suitable slots available`);
      }
    }
  }

  result.success = result.unassigned.length === 0 || (allowOverflow && result.assignments.length > 0);

  return result;
}

/**
 * Optimize slot assignment (reorder for better balance)
 */
 ...
 */
export function detectSlotConflicts(assignments: SlotAssignment[]): Array<{
  slot_key: string;
  facilities: string[];
  conflict: string;
}> {
 ...
 */
function sortFacilitiesByRules(facilities: Facility[], rules: AssignmentRule[]): Facility[] {
...
export function suggestOptimalVehicle(
  ...
  for (const vehicle of availableVehicles) {
    ...
    const weightUtilization = totalWeight / vehicleCapacityKg;
    const volumeUtilization = totalVolume / vehicleCapacityM3;
