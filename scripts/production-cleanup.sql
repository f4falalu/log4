...
BEGIN
  ...
  AND state = 'kano'
  AND NOT EXISTS (
    -- Keep LGAs that have facilities assigned
    SELECT 1 FROM facilities WHERE facilities.lga_id = lgas.id
  );
...
BEGIN
  -- Count placeholder warehouses (repeating digit patterns)
  SELECT COUNT(*) INTO placeholder_warehouses
  FROM warehouses
  WHERE id::text ~ '[0-9]{8}-\1{4}-\1{4}-\1{4}-\1{12}';

  -- Count placeholder facilities (f + repeating pattern)
  SELECT COUNT(*) INTO placeholder_facilities
  FROM facilities
  WHERE id::text LIKE 'f%-%-%-%-%'
  AND (
    id::text LIKE '%1111111%'
    OR id::text LIKE '%2222222%'
    OR id::text LIKE '%3333333%'
    OR id::text LIKE '%4444444%'
    OR id::text LIKE '%5555555%'
  );
