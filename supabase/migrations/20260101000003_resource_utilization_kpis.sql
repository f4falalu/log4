-- Resource Utilization KPIs Migration
-- This migration adds 6 analytics functions for tracking resource utilization metrics

-- =============================================
-- 1. Vehicle Payload Utilization
-- =============================================
-- Tracks both payload capacity (items/capacity) and weight (actual_weight/max_weight) utilization
...
-- =============================================
-- Tracks delivery performance by health program (Family Planning, DRF, HIV/AIDS, Malaria)
...
-- =============================================
-- Tracks deliveries per driver to identify underutilization
...
-- 4. Route Efficiency
-- =============================================
-- Compares actual vs estimated distance and duration
...
-- 5. Facility Coverage
-- =============================================
-- Calculates facility delivery coverage and identifies gaps
...
-- 6. Cost by Program
-- =============================================
-- Calculates cost per delivery segmented by health program
...
BEGIN
  ...
  SELECT
    ...
    ROUND((pc.fuel + pc.maint) / NULLIF(pc.distance, 0), 2) as cost_per_km
...
-- =============================================
-- Create public wrappers for all analytics functions following existing pattern
...
-- GIN index for facility_ids array operations
CREATE INDEX IF NOT EXISTS idx_delivery_batches_facility_ids_gin
ON delivery_batches USING GIN (facility_ids);

-- Index for program filtering (skip if already exists from other migration)
-- CREATE INDEX IF NOT EXISTS idx_facilities_programme
-- ON facilities (programme)
-- WHERE programme IS NOT NULL AND deleted_at IS NULL;

-- Composite index for delivery batch filtering
CREATE INDEX IF NOT EXISTS idx_delivery_batches_status_dates
ON delivery_batches (status, actual_end_time)
WHERE status = 'completed';

-- Index for vehicle joins
CREATE INDEX IF NOT EXISTS idx_delivery_batches_vehicle_id
ON delivery_batches (vehicle_id)
WHERE status = 'completed';

-- Index for driver joins
...
COMMENT ON FUNCTION analytics.get_vehicle_payload_utilization IS 'Calculates vehicle payload utilization (items/capacity) and weight utilization (weight/max_weight) metrics';
COMMENT ON FUNCTION analytics.get_program_performance IS 'Tracks delivery performance by health program (Family Planning, DRF, HIV/AIDS, Malaria)';
COMMENT ON FUNCTION analytics.get_driver_utilization IS 'Measures driver utilization based on deliveries per week with status categorization';
COMMENT ON FUNCTION analytics.get_route_efficiency IS 'Compares actual vs estimated route metrics (distance and duration)';
COMMENT ON FUNCTION analytics.get_facility_coverage IS 'Calculates facility delivery coverage percentage and identifies service gaps';
COMMENT ON FUNCTION analytics.get_cost_by_program IS 'Aggregates delivery costs by health program for budget analysis';
